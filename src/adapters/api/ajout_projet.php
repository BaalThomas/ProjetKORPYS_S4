...
function getProjetById(int $id_projet): array {
    ...
    $res = $db->prepare("SELECT * FROM PROJET WHERE id_projet = :id_projet");
...
if (!isset($_POST['id_user']) || !isset($_POST['nom_projet']) || !isset($_POST['description_projet']) || !isset($_POST['date_max'])) {
    $json["status"] = "error";
...
} while (!empty(getProjetById($id_projet)["data"]));

$res = $db->prepare("INSERT INTO PROJET (`id_projet`, `id_user`, `nom_projet`, `description_projet`, `status_projet`, `date_debut`, `date_max`) VALUES (:id_projet, :id_user, :nom_projet, :description_projet, :status_projet, :date_debut, :date_max)");
